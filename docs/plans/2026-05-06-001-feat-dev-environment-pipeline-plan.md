...
- **Does `connect-pg-simple` need the session table pre-created?** No. `createTableIfMissing: true` is set in `server/auth.ts` (line 31). The session table auto-creates on first request. However, all other tables (users, campaigns, pairs, etc.) require `db:push`.
